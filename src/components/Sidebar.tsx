    ...
    <aside className="w-64 bg-[var(--bg-card)] border-r border-[var(--border-subtle)] flex flex-col h-screen sticky top-0 font-sans">
      ...
      <div className="flex-1 overflow-y-auto py-4">
        <nav className="space-y-1 px-3">
          ...
          <NavItem icon={<PieChart size={20} />} label={t('nav.overview')} active={activeTab === 'Overview'} onClick={() => setActiveTab('Overview')} />
          <NavItem icon={<BarChart2 size={20} />} label={t('nav.valuation')} active={activeTab === 'Valuation'} onClick={() => setActiveTab('Valuation')} />
          <NavItem icon={<TrendingUp size={20} />} label={t('nav.futureGrowth')} active={activeTab === 'Future Growth'} onClick={() => setActiveTab('Future Growth')} />
          <NavItem icon={<Activity size={20} />} label={t('nav.pastPerformance')} active={activeTab === 'Past Performance'} onClick={() => setActiveTab('Past Performance')} />
          <NavItem icon={<Shield size={20} />} label={t('nav.financialHealth')} active={activeTab === 'Financial Health'} onClick={() => setActiveTab('Financial Health')} />
