        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-subtle text-xs text-secondary">
              <th className="pb-2 font-bold text-primary text-sm">{formattedDate}</th>
            ...
            {Object.entries(metricsConfig).map(([key, config]) => {
              ...
              return (
                <tr key={key}>
                  ...
                  {dataPoint.isForecast && (
                    <>
                      <td className="py-1 text-right text-secondary">{value !== null && value !== undefined ? dataPoint.analysts : 'n/a'}</td>
                      <td className="py-1 text-right text-secondary text-xs">{value !== null && value !== undefined ? dataPoint.lastUpdated : 'n/a'}</td>
                    </>
                  )}
                </tr>
              );
...
export function EarningsRevenueForecast() {
  ...
  return (
    <div className="mb-16" id="section_2_1">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-primary mb-2">2.1 Earnings and Revenue Growth Forecasts</h2>
      </div>
      ...
      <div className="surface-card p-6">
            ...
            <div className="h-[400px] w-full relative">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={forecastData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  ...
                  {/* Background Shading */}
                  {/* @ts-ignore */}
                  <ReferenceArea x1="2023-03-31" x2="2025-12-31" fill="var(--bg-base)" fillOpacity={0.3} />
                  {/* @ts-ignore */}
        ...
        <div className="flex justify-end mt-6 space-x-2 border-t border-subtle pt-4">
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-card hover:bg-subtle border border-subtle rounded-lg text-sm text-secondary font-semibold transition-colors shadow-sm"
          >
